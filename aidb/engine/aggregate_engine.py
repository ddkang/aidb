...
class ApproximateAggregateEngine(BaseEngine):
    ...
    async def get_sampled_blobs_with_stats(self,
          infer_output, agg_on_column, agg_table, query, est_conn):
      sampled_blobs = []
    ...
    async def execute_aggregate_query(self, query: Query, dialect=None):
      ...
      self.num_samples_required = NUM_PILOT_SAMPLES

      service_ordering = self._config.inference_topological_order
      inference_services_executed = set()
      input_samples, agg_service = defaultdict(lambda: []), None
      ...
      async with self._sql_engine.begin() as conn:
        all_blobs = await self.get_sampled_blobs_with_stats(
            all_samples, agg_on_column, agg_on_column_table, query, conn)
