...
class QueryParameters(str, Enum):
...
class QueryParsingTests(IsolatedAsyncioTestCase):
  ...
  def _test_query(self, test_query, config):
    ...
    assert len(query.all_queries_in_expressions) == test_query[QueryParameters.NUM_OF_SELECT_CLAUSES]
    self.assertEqual(query.query_after_normalizing.sql_str, test_query[QueryParameters.NORMALIZED_QUERY_STR])
    and_fp = []
    for and_connected in query.filtering_predicates:
      ...
      and_fp.append(or_fp)
    self.are_lists_equal(and_fp, test_query[QueryParameters.CORRECT_FP])
    self.are_lists_equal(query.inference_engines_required_for_filtering_predicates, test_query[QueryParameters.CORRECT_SERVICE])
    self.are_lists_equal(query.tables_in_filtering_predicates, test_query[QueryParameters.CORRECT_TABLES])
  ...
  async def test_nested_query(self):
    ...
    "test_query_0": {
        QueryParameters.QUERY_STR: '''
                    ...
                    ''',
        QueryParameters.NORMALIZED_QUERY_STR: ("SELECT colors02.color "
                                "FROM colors02 LEFT JOIN objects00 ON colors02.frame = objects00.frame "
        ...
        QueryParameters.CORRECT_FP: [['colors02.color IN (SELECT table2.color AS alias_color2 FROM colors02 AS table2)',
        ...
        QueryParameters.CORRECT_SERVICE: [{'colors02', 'objects00'}],
        QueryParameters.CORRECT_TABLES: [{'colors02', 'objects00'}],
        QueryParameters.NUM_OF_SELECT_CLAUSES: 3
    },
    # test table alias
    "test_query_1": {
        QueryParameters.QUERY_STR: '''
                    ...
                    ''',
        # test table alias
        QueryParameters.NORMALIZED_QUERY_STR: ("SELECT colors02.color FROM colors02 "
                                "WHERE colors02.frame IN (SELECT * FROM blobs_00) "
                                "AND colors02.object_id > 0"),
        # test replacing column with root column in filtering predicate,
        # the root column of 'colors02.object_id' is 'objects00'
        QueryParameters.CORRECT_FP: [['blobs_00.frame IN (SELECT * FROM blobs_00)'],
                   ['objects00.object_id > 0']],
        # filter predicates connected by AND are in different set
        QueryParameters.CORRECT_SERVICE: [set(), {'objects00'}],
        QueryParameters.CORRECT_TABLES: [set(), {'objects00'}],
        QueryParameters.NUM_OF_SELECT_CLAUSES: 2
    },
    # test sub-subquery
     "test_query_2": {
        QueryParameters.QUERY_STR: '''SELECT frame, object_id FROM colors02 AS cl
                        WHERE cl.object_id > (SELECT AVG(object_id) FROM objects00
                                              WHERE frame > (SELECT AVG(frame) FROM blobs_00 WHERE frame > 500))
                    ''',
        QueryParameters.NORMALIZED_QUERY_STR: ("SELECT colors02.frame, colors02.object_id FROM colors02 "
                                "WHERE colors02.object_id > (SELECT AVG(object_id) FROM objects00 WHERE frame > "
                                "(SELECT AVG(frame) FROM blobs_00 WHERE frame > 500))"),
        # test replacing column with root column in filtering predicate,
        # the root column of 'colors02.object_id' is 'objects00'
        QueryParameters.CORRECT_FP: [["objects00.object_id > (SELECT AVG(object_id) FROM objects00 "
                   "WHERE frame > (SELECT AVG(frame) FROM blobs_00 WHERE frame > 500))"]],
        # filter predicates connected by AND are in different set
        QueryParameters.CORRECT_SERVICE: [{'objects00'}],
        QueryParameters.CORRECT_TABLES:  [{'objects00'}],
        QueryParameters.NUM_OF_SELECT_CLAUSES: 3
    },
    # test multiple aliases
     "test_query_3": {
        QueryParameters.QUERY_STR: '''
                    ...
                    ''',
        QueryParameters.NORMALIZED_QUERY_STR: ("SELECT colors02.color, objects00.x_min "
                                "FROM colors02 LEFT JOIN objects00 ON colors02.frame = objects00.frame "
        ...
        QueryParameters.CORRECT_FP: [["colors02.color IN (SELECT table3.color AS alias_color2 FROM colors02 AS table3)"],
     ...
     "test_query_4":{
        ...
        QueryParameters.NORMALIZED_QUERY_STR: ("SELECT colors02.color, objects00.x_min "
                                ...
                                "WHERE (SELECT table3.color AS alias_color2 FROM colors02 AS table3) "
                                "> (SELECT AVG(ob.object_id) FROM objects00 AS ob WHERE frame > 500)"),
        QueryParameters.CORRECT_FP: [["(SELECT table3.color AS alias_color2 FROM colors02 AS table3) > "
                   "(SELECT AVG(ob.object_id) FROM objects00 AS ob WHERE frame > 500)"]],
      
        QueryParameters.CORRECT_SERVICE: [{}],
        QueryParameters.CORRECT_TABLES:[{}],
        QueryParameters.NUM_OF_SELECT_CLAUSES: 3
    },
     "test_query_5":{
        QueryParameters.QUERY_STR:'''
                    ...
                    FROM colors02 table1 LEFT JOIN objects00 table2 ON table1.frame = table2.frame;
                    ''',
        QueryParameters.NORMALIZED_QUERY_STR: ("SELECT colors02.color, objects00.x_min, objects00.y_min "
                                "FROM colors02 LEFT JOIN objects00 ON colors02.frame = objects00.frame"),
        QueryParameters.CORRECT_FP: [[]],
        QueryParameters.CORRECT_SERVICE: [{}],
        QueryParameters.CORRECT_TABLES: [{}],
        QueryParameters.NUM_OF_SELECT_CLAUSES: 1
    },
     "test_query_6":{
        QueryParameters.QUERY_STR: '''
                    ...
                    ''',
        QueryParameters.NORMALIZED_QUERY_STR: ("SELECT colors02.color, objects00.x_min, blobs_00.frame "
                                "FROM colors02 LEFT JOIN objects00 ON colors02.frame = objects00.frame "
     ...
     "test_query_7":{
        QueryParameters.QUERY_STR: '''
                    ...
                    ''',
        QueryParameters.NORMALIZED_QUERY_STR:  ("SELECT colors02.color, objects00.x_min, colors02.frame "
                                "FROM colors02 JOIN objects00 ON colors02.frame = objects00.frame "
                                "WHERE colors02.color = 'blue' AND objects00.x_min > 600"),
        QueryParameters.CORRECT_FP: [["colors02.color = 'blue'"], ['objects00.x_min > 600']],
        QueryParameters.CORRECT_SERVICE: [{'colors02'}, {'objects00'}],
        QueryParameters.CORRECT_TABLES: [{'colors02'}, {'objects00'}],
        QueryParameters.NUM_OF_SELECT_CLAUSES: 1
    },
     "test_query_8":{
        QueryParameters.QUERY_STR:'''
                    ...
                    ''',
        QueryParameters.NORMALIZED_QUERY_STR: ("SELECT colors02.color, userfunction(objects00.x_min, objects00.y_min, "
                                "objects00.x_max, objects00.y_max) "
                                "FROM colors02 JOIN objects00 ON colors02.frame = objects00.frame "
                                "WHERE objects00.frame > 10000 OR objects00.y_max < 800"),
        QueryParameters.CORRECT_FP: [['blobs_00.frame > 10000', 'objects00.y_max < 800']],
        QueryParameters.CORRECT_SERVICE: [{'objects00'}],
        QueryParameters.CORRECT_TABLES: [{'objects00'}],
        QueryParameters.NUM_OF_SELECT_CLAUSES: 1
    }
  ...
  async def test_correct_approximate_query(self):
    ...
    "test_query_0": {
        QueryParameters.QUERY_STR: '''
                 ...
                 ''',
        QueryParameters.NORMALIZED_QUERY_STR: ("SELECT AVG(objects00.x_min) "
                             "FROM objects00 "
                             "WHERE objects00.frame > (SELECT AVG(frame) FROM blobs_00) ERROR_TARGET 10% CONFIDENCE 95%"),
 
        QueryParameters.CORRECT_FP : [["blobs_00.frame > (SELECT AVG(frame) FROM blobs_00)"]],
     ...
     "test_query_1":{
        QueryParameters.QUERY_STR: '''
                 ...
                 CONFIDENCE 95%;
                 ''',
        QueryParameters.NORMALIZED_QUERY_STR: ("SELECT colors02.frame FROM colors02 "
                             "WHERE colors02.color IN (SELECT color FROM colors02 WHERE frame > 10000) "
                             "RECALL_TARGET 80% "
                             "CONFIDENCE 95%"),
        QueryParameters.CORRECT_FP:[["colors02.color IN (SELECT color FROM colors02 WHERE frame > 10000)"]],
        QueryParameters.CORRECT_SERVICE: [{'colors02'}],
        QueryParameters.CORRECT_TABLES: [{'colors02'}],
        QueryParameters.NUM_OF_SELECT_CLAUSES: 2
    }
}
    for i in range(len(queries)):
